                    ...
                    <div class="product__details__images module-gallery">
                        ...
                        <div class="row slider-wrapper mt-1">
                            <ul class="col-3 slider-thumb noPad noMar">
                                ...
                                @if ($image != null) <img src="{{ $image != null ? asset($image) : '' }}" alt=""></li>
                                @endif
                                @endforeach
                                @endif
                            </ul>
                            ...
                            <ul class="col-12 col-md-9 slider-preview noPad noMar">
                                ...
                                @foreach ($images as $image)
                                @if ($image != null)
                                <li class="type-image"><img src="{{ $image != null ? asset($image) : '' }}" alt=""></li>
                                @endif
                                @endforeach
                                @endif
                            </ul>
                        ...
                        <div class="slider-wrapper d-none">
                            ...
                            <ul class="slider-preview">
                                ...
                                @foreach ($images as $image)
                                @if ($image != null)
                                <li class="type-image"><img style="border:1px solid #ddd;" src="{{ $image != null ? asset($image) : '' }}" alt=""></li>
                                @endif
                                @endforeach
                                @endif
                            </ul>
                    ...
                    <form id="add-to-cart-form">
                        ...
                        <div class="products-details-desc">
                            <div class="wishlist-btn details__wishlist-btn d-block d-md-none">
                                <a href="javascript:void(0)" class="add_to_wishlist" data-product-id="{{ $product->id }}" id="add_to_wishlist_{{ $product->id }}">
                                    @php
                            ...
                            <div class="price">
                                ...
                                @else
                                <span class="new-price">{{ Helper::currency_converter($product->purchase_price) }}</span>
                                @endif
                            ...
                            <div class="products-color-select mt-2 d-md-none d-block color-select" data-toggle="modal" data-target="#AllColors">
                            ...
                            <div class="products-color-switch d-md-block d-none">
                                <ul class="color-list-row">
                                    @foreach (json_decode($product->colors) as $key => $color)
                                    <li>
                                        <label class="aiz-megabox pl-0 mr-2" data-toggle="tooltip" data-placement="bottom" data-title="{{ \App\Models\AttributeValue::where('color_code', $color)->first()->name }}">
                                            <input required type="radio" name="color" value="{{ $color }}" @if ($key==0) checked @endif>
                                            <span class="aiz-megabox-elem rounded d-flex align-items-center justify-content-center mb-2">
                                                <span class="size-30px d-inline-block color-rounded" style="background: {{ $color }};"></span>
                                            </span>
                                        </label>
                                    </li>
                                    @endforeach
                            ...
                            <div class="products-size-wrapper">
                                    ...
                                    <ul class="checkbox-alphanumeric checkbox-alphanumeric--style-1 ">
                                        ...
                                        <li class="pl-0 mr-0">
                                            <label style="margin-right: -20px;">
                                                <input type="radio" required id="size_item" name="attribute_id_{{ $choice->attribute_id }}" value="{{ $option }}" @if ($key==0) checked @endif>
                                ...
                                <div class="products-details-title d-flex justify-content-between d-md-none d-block">
                                    <span class="float-left">Size:</span>
                                </div>
                                <div class="clearfix"></div>
                                <div class="products-color-select mt-2 d-md-none d-block color-select" data-toggle="modal" data-target="#AllSizes">
                                    <select class="form-control" id="mobile-select-size" disabled>
                                        @foreach ($choice->values as $key => $option)
                                        <option value="{{ $option }}">
                                            {{ $option }}
                                        </option>
                            ...
                            <div class="products-add-to-cart mt-3 row">
                                ...
                                <div class="input-counter">
                                    <select type="select" class="form-control" name="variant" id="variant" style="top: 5px;height: 50px !important;">
                                        @foreach($product->stocks AS $key => $st)
                                        <option value="{{$st->id}}" data-price="{{$st->price}}" {{($key==0)?'selected':''}}>{{$st->variant}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @endif
                                ...
                                <div class="input-counter">
                                    <span class="minus-btn btn-number" data-type="minus" data-field="quantity"><i class='bx bx-minus'></i></span>
                                    <input type="text" class="input-number" name="quantity" value="1" min="1" max="100" style="height:40px">
                                    <span class="plus-btn btn-number" data-type="plus" data-field="quantity"><i class='bx bx-plus'></i></span>
                                </div>

                                <!-- Add to Cart Button -->
                                <button type="button" onclick="addToCart()" class="default-btn  secondary-btn add-to-cart-btn d-none d-lg-inline-block"> Add to
            ...
            <div class="modal-body modal-allColors p-0">
                <div class="d-flex justify-content-between">
                    <div class="img-wrapper">
                        <img src="{{ $product->featured_image != null ? asset($product->featured_image) : Helper::DefaultImage() }}" alt="">
                    </div>
                ...
                <div class="all-colors-list">
                    ...
                    <div class="products-color-switch">
                        <ul class="color-list-row">
                            @foreach (json_decode($product->colors) as $key => $color)
                            <li>
                                <label class="aiz-megabox pl-0 mr-2" data-title="{{ \App\Models\AttributeValue::where('color_code', $color)->first()->name }}">
                                    <input type="radio" name="color" class="product_detail_color_pick" value="{{ $color }}" data-color_code="{{ $color }}" data-color_name="{{ \App\Models\AttributeValue::where('color_code', $color)->first()->name }}" @if ($key==0) checked @endif>
                                    <span class="aiz-megabox-elem rounded d-flex align-items-center justify-content-center mb-2">
                                        <span class="size-30px d-inline-block color-rounded" style="background: {{ $color }}"></span>
                                    </span>
                                </label>
        ...
        <div class="modal-content h-100">
            <div class="modal-body modal-allSizes p-0">
                <div class="d-flex justify-content-between">
                    ...
                    <div class="content-wrapper w-100">
                        <div class="text-right">
                            <button type="button" data-dismiss="modal" class="btn secondary-btn btn-save text-white">
                                <i class="bx bx-check bx-flashing"></i>
                                Save
                            </button>
                        </div>
                ...
                <div class="all-colors-list">
                    <ul class="checkbox-alphanumeric checkbox-alphanumeric--style-2 row">
                        ...
                        <li class="pl-0 mr-0 col-6">
                            <label class="d-block">
                                <input type="radio" required class="product_detail_size_pick" data-size="{{ $option }}" data-attribute_id="{{ $choice->attribute_id }}" name="attribute_id_{{ $choice->attribute_id }}" value="{{ $option }}" @if ($key==0) checked @endif>
                                <span class="aiz-megabox-elem">{{ $option }}</span>
                            </label>
                        </li>
                        @endforeach
...
<script>
    window.addEventListener('load', function() {
        // var amount = $("#variant option:first").attr('data-price');
        // $('.chosen_price').text('₹ ' + amount);
        getVariantPrice();
    });
    //cart counter
...
<script>
    getVariantPrice();
    $('#add-to-cart-form input,#variant').on('change', function() {
        getVariantPrice();
    });
