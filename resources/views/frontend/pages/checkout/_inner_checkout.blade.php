        ...
        <div class="col-lg-12 mb-6">
            ...
            <div class="row">
                ...
                <div class="col text-left">
                    ...
                    <input required type="text" class="form-control" id="firstName" name="first_name" placeholder="Full name" value="{{ $name[0] ?? old('first_name') }}" />
                </div>
                <div class="col text-left">
                    <label>Last Name *</label>
                    <input required type="text" class="form-control" id="lastName" name="last_name" placeholder="Last name" value="{{ $name[1] ?? old('last_name') }}" />
                </div>
            </div>
            <div class="row">
                <div class="col text-left">
                    <label>Email *</label>
                    <input required type="email" class="form-control" id="email" name="email" placeholder="Email" value="{{ $user->email ?? old('email') }}" />
                </div>
                <div class="col text-left">
                    <label>Phone *</label>
                    <input required type="text" class="form-control" id="phone" name="phone" placeholder="Phone" value="{{ $user->phone  ?? old('phone')}}" />
                </div>
            </div>
            ...
            <div class="row">
                <div class="col text-left">
                    ...
                    <select class="form-control" name="address_list" id="address_list">
                        <option disabled selected>Selecte Address</option>
                        @foreach($address AS $key => $ad)
                        <option value="{{$ad->id}}">{{strtoupper($ad->con_add)}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col text-left">
                    ...
                    <input required type="text" class="form-control" id="country" name="country" placeholder="Country" value="{{ $user->country ?? old('country') }}" />
                </div>
            </div>
            <div class="row">
                <div class="col text-left">
                    <label>Street Address *</label>
                    <input required type="text" class="form-control" id="address" name="address" value="{{ $user->address ?? old('address') }}" placeholder="House number and Street name" />
                    <input type="text" class="form-control" id="address2" name="address2" value="{{ $user->address2 ?? old('address') }}" placeholder="Appartments, suite, unit etc ..." />
                </div>
            </div>
            <div class="row">
                <div class="col text-left">
                    <label>State / Province </label>
                    <input type="text" class="form-control" id="state" placeholder="State" name="state" value="{{$user->state ?? old('state') }}" />
                </div>
                <div class="col text-left">
                    <label>Postcode / ZIP </label>
                    <input type="text" class="form-control" id="postcode" placeholder="Postcode" name="postcode" value="{{$user->postcode ?? old('postcode') }}" />
                </div>
            </div>
            @endif
            ...
            <div class="shipping_address">
                ...
                <div class="row">
                    <div class="col text-left">
                        ...
                        <input type="text" class="form-control" id="scountry" name="scountry" placeholder="Country" value="{{ $user->scountry ?? old('scountry') }}" />
                    </div>
                </div>
                <div class="row">
                    <div class="col text-left">
                        <label for="saddress">Street Address *</label>
                        <input type="text" class="form-control" id="saddress" name="saddress" value="{{$user->saddress ??  old('saddress') }}" placeholder="House number and Street name" />
                        <input type="text" class="form-control" id="saddress2" name="saddress2" value="{{$user->saddress2 ??  old('saddress2') }}" placeholder="Appartments, suite, unit etc ..." />
                    </div>
                </div>
                <div class="row">
                    <div class="col text-left">
                        <label>State / Province </label>
                        <input type="text" class="form-control" id="sstate" placeholder="State" name="sstate" value="{{$user->sstate ??  old('sstate') }}" />
                    </div>
                    <div class="col text-left">
                        <label>Postcode / ZIP </label>
                        <input type="text" class="form-control" id="spostcode" placeholder="Postcode" name="spostcode" value="{{$user->spostcode ??  old('spostcode') }}" />
                    </div>
                </div>
                            ...
                            <tr>
                                <td class="product-thumbnail">
                                    <a href="{{route('product.detail',$cartItem['slug'])}}">
                                        <img style="border:1px solid #ddd" src="{{asset($cartItem['image'])}}" alt="item">
                                    </a>
                                </td>
                                <td class="product-name pl-2">
                                    <a href="{{route('product.detail',$cartItem['slug'])}}">{{ucfirst($product_name_with_choice)}}</a>
                                </td>
                                <td class="product-quantity"><span class="unit-amount">{{$cartItem['quantity']}}</span></td>
                                <td class="product-price">
                                    <span class="unit-amount">{{Helper::currency_converter($cartItem['quantity']*$cartItem['price'])}}</span>
                                </td>
                            </tr>
                            @endforeach
                            @endif
